...
public class HoverController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (hoverDisabled)
        {
            wheelL1Collider.motorTorque = 0;
            wheelL2Collider.motorTorque = 0;
            wheelR1Collider.motorTorque = 0;
            wheelR2Collider.motorTorque = 0;
            return;
        }
        ...
        if (Input.GetKeyDown(KeyCode.F))
        {
            ...
            Player.Instance.characterController.enabled = false; // Disable the CharacterController temporarily to set position
            Player.Instance.transform.position = transform.position + (Vector3.left*5f);
            Player.Instance.characterController.enabled = true; // Re-enable the CharacterController
        }
    }
